...
export default function Command() {
	...
	const { sortedEntries, visitItem } = useAliasesWithFrecency(
	...
	useEffect(() => {
		if (sortedEntries.length === 1 && searchText.trim()) {
			...
			const isSnippetMode =
				snippetPrefix && searchText.startsWith(snippetPrefix);

			if (isSnippetMode) {
				const valueToInsert = getRandomizedValue(
					aliasItem.value,
...
function AliasListItem({
	...
	const originalAlias = isSnippetMode
		? alias.slice(snippetPrefix.length)
		: alias;
